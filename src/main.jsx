...
function Login({ onSession }) {
  ...
  return <div className="login"><div className="card"><h2>Entrar na viagem ✈️</h2><p className="muted">Login individual para Vanessa, Camila e Danielle.</p>
    ...
    <form onSubmit={submit}>
      ...
      <label className="field">Senha<input type="password" minLength="6" value={password} onChange={e=>setPassword(e.target.value)} required /><small className="muted">Mínimo de 6 caracteres.</small></label>
...
function ClaimParticipant({ session, data, reload }) {
  ...
  return <main className="main wrap"><section className="grid"><div className="card"><h2>Vincular sua conta</h2><p className="muted">Escolha quem é você na viagem. Cada conta fica vinculada a uma participante.</p><div className="navCards">{available.map(p=><div className="mini" key={p.id}><h3>{p.name}</h3><p className="muted">Cofrinho individual e histórico próprio.</p><button className="btn" disabled={busy} onClick={()=>claim(p)}>Sou eu</button></div>)}</div>{available.length === 0 && <div className="error">Todas as participantes já estão vinculadas. A administradora pode liberar um vínculo na aba Perfil.</div>}</div></section></main>
}
...
function Dashboard({ data, visibleParticipants, isAdmin }) {
  ...
  return <section className="grid"><div className="card wide"><h2>Dashboard</h2><p className="muted">{isAdmin ? 'Visão administradora: grupo completo ou participante selecionada.' : 'Sua visão individual.'}</p><div className="navCards"><div className="mini"><b>Faltam</b><div className="big">{daysUntil('2027-10-08T20:00')}</div><span>dias</span></div><div className="mini"><b>Meta exibida</b><div className="money">{money(groupTarget)}</div></div><div className="mini"><b>Guardado</b><div className="money">{money(groupSaved)}</div></div><div className="mini"><b>Pendente</b><div className="money">{money(groupPending)}</div></div></div></div>{stats.map(x=><div className="card third" key={x.participant.id}><h2>{x.participant.name}</h2><p className="muted">Meta mensal: {money(x.monthly)}</p><p><b>Meta:</b> {money(x.target)}<br/><b>Guardado:</b> {money(x.saved)}<br/><b>Pendente:</b> {money(x.pending)}</p><ProgressBar value={x.progress}/><p className="muted">{x.progress.toFixed(1)}% concluído</p></div>)}</section>
}
...
function ProfilePage({ session, data, reload, isAdmin, myParticipant, adminView, setAdminView }) {
  ...
  async function changeMine(id) {
    if (!id) return
    setBusy(true)
    await supabase.from('participants').update({ profile_id: null, email: null }).eq('profile_id', session.user.id)
    const { error } = await supabase.from('participants').update({ profile_id: session.user.id, email: session.user.email }).eq('id', id)
    setBusy(false)
    if (error) return alert(error.message)
    await reload()
  }

  async function releaseParticipant(p) {
    if (!confirm(`Liberar o perfil ${p.name}?`)) return
    const { error } = await supabase.from('participants').update({ profile_id: null, email: null }).eq('id', p.id)
    if (error) return alert(error.message)
    await reload()
  }

  return <section className="grid"><div className="card half"><h2>Meu perfil</h2><p><b>E-mail:</b> {session.user.email}<br/><b>Papel:</b> {isAdmin ? 'Administradora' : 'Participante'}<br/><b>Participante:</b> {myParticipant?.name || 'não vinculado'}</p><label className="field">Trocar meu perfil<select disabled={busy} value={myParticipant?.id || ''} onChange={e=>changeMine(e.target.value)}><option value="">Selecione</option>{available.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><small className="muted">Use apenas se tiver escolhido o perfil errado.</small></label></div>{isAdmin && <div className="card half"><h2>Controle admin</h2><label className="field">Visualizar cofrinhos como<select value={adminView} onChange={e=>setAdminView(e.target.value)}><option value="all">Grupo completo</option>{data.participants.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><div className="notice"><b>Assistente da viagem:</b> por enquanto você me usa aqui no ChatGPT. Depois criamos uma área de assistente dentro do app.</div></div>}{isAdmin && <div className="card"><h2>Perfis vinculados</h2><table className="table"><thead><tr><th>Participante</th><th>E-mail</th><th>Ação</th></tr></thead><tbody>{data.participants.map(p=><tr key={p.id}><td>{p.name}</td><td>{p.email || 'livre'}</td><td>{p.profile_id ? <button className="btn danger" onClick={()=>releaseParticipant(p)}>Liberar</button> : <span className="status ok">Livre</span>}</td></tr>)}</tbody></table></div>}</section>
}
...
function Checklist({ data, reload, visibleParticipants, isAdmin }) {
  const visibleIds = visibleParticipants.map(p=>p.id)
...
function SimpleList({ title, items, render }) {
  return <section className="grid"><div className="card"><h2>{title}</h2>{items.map(render)}</div></section>
}
